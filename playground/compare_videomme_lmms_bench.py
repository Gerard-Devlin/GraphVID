...
def _phase_name(method: str) -> str:
    return "ours" if method in {"talon", "apexvid"} else method


def _bench_record_phase_name(method: str) -> str:
    return "ours" if method in {"fastvid", "visionzip", "fastgraphvid", "curvevid", "talon", "apexvid"} else method
...
def _find_bench_jsonl(summary_path: Path, method: str) -> Path | None:
    ...
    if method in {"fastvid", "visionzip", "fastgraphvid", "curvevid", "apexvid"}:
