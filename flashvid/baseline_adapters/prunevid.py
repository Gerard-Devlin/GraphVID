...
def _dpc_cluster(features: torch.Tensor, cluster_count: int, neighbors: int = 7):
...
def _cluster_average(features: torch.Tensor, cluster_count: int):
    labels, centers = _dpc_cluster(features, cluster_count=cluster_count)
...
def _temporal_windows(frame_features: torch.Tensor, segment_ratio: float) -> list[tuple[int, int]]:
    ...
    labels, _ = _dpc_cluster(frame_features, cluster_count=cluster_count)
...
def prunevid_compression(
    ...
    windows = _temporal_windows(frame_means, segment_ratio)
    ...
    for start, end in windows:
        ...
        if static_locations.numel() > 0:
            ...
            if static_clusters > 14:
                ...
                static_features, centers = _cluster_average(static_features, static_clusters)
        ...
        for local_frame, frame_idx in enumerate(range(start, end)):
            ...
            if dynamic_clusters > 14:
                ...
                dynamic_features, centers = _cluster_average(dynamic_features, dynamic_clusters)
