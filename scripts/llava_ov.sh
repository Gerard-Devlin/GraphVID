...
CERTSB_TOKEN_SELECTION_METHOD="${CERTSB_TOKEN_SELECTION_METHOD:-$CERTV3_TOKEN_SELECTION_METHOD}"
CERTSB_SEMANTIC_RATIO="${CERTSB_SEMANTIC_RATIO:--1.0}"
CERTSB_TEMPORAL_RATIO="${CERTSB_TEMPORAL_RATIO:--1.0}"
CERTSB_COVERAGE_RATIO="${CERTSB_COVERAGE_RATIO:--1.0}"
CERTSB_TEMPORAL_BINS="${CERTSB_TEMPORAL_BINS:-16}"
CERTSB_LOCAL_RADIUS="${CERTSB_LOCAL_RADIUS:-1}"
CERTSB_CHANGE_LOW_QUANTILE="${CERTSB_CHANGE_LOW_QUANTILE:-0.50}"
CERTSB_CHANGE_PEAK_QUANTILE="${CERTSB_CHANGE_PEAK_QUANTILE:-0.75}"
CERTSB_CHANGE_HIGH_QUANTILE="${CERTSB_CHANGE_HIGH_QUANTILE:-0.90}"
CERTSB_SCENE_QUANTILE="${CERTSB_SCENE_QUANTILE:-0.90}"
CERTSB_SCENE_SUPPRESSION="${CERTSB_SCENE_SUPPRESSION:-0.65}"
CERTSB_TEMPORAL_QUERY_WEIGHT="${CERTSB_TEMPORAL_QUERY_WEIGHT:-0.15}"
CERTSB_PROTECT_STRUCTURED="${CERTSB_PROTECT_STRUCTURED:-True}"
CERTSB_DEBUG="${CERTSB_DEBUG:-False}"
...
method_args() {
  ...
  case "$method" in
    ...
    certvid_hr)
      ...
      ;;
    certvid_sb)
      printf 'compression_variant=certvid_sb,token_selection_method=%s,certv3_budget_uses_expansion=%s,certv3_query_atoms=%s,certv3_temporal_bins=%s,certv3_spatial_bins=%s,certv3_candidate_multiplier=%s,certv3_query_weight=%s,certv3_track_threshold=%s,certv3_spatial_penalty=%s,certv3_metric_dim=%s,certv3_frame_coverage_ratio=%s,certv3_cell_coverage_ratio=%s,certv3_query_threshold=%s,certv3_query_per_atom=%s,certv3_structural_weight=%s,certv3_whitening_strength=%s,certv3_quality_floor=%s,certv3_ridge=%s,certv3_swap_steps=%s,certv3_swap_pool=%s,certv3_swap_margin=%s,certv3_fusion_alpha=%s,certv3_assignment_temperature=%s,certsb_semantic_ratio=%s,certsb_temporal_ratio=%s,certsb_coverage_ratio=%s,certsb_temporal_bins=%s,certsb_local_radius=%s,certsb_change_low_quantile=%s,certsb_change_peak_quantile=%s,certsb_change_high_quantile=%s,certsb_scene_quantile=%s,certsb_scene_suppression=%s,certsb_temporal_query_weight=%s,certsb_protect_structured=%s,certsb_debug=%s' \
        "$CERTSB_TOKEN_SELECTION_METHOD" "$CERTV3_BUDGET_USES_EXPANSION" "$CERTV3_QUERY_ATOMS" "$CERTV3_TEMPORAL_BINS" "$CERTV3_SPATIAL_BINS" "$CERTV3_CANDIDATE_MULTIPLIER" "$CERTV3_QUERY_WEIGHT" "$CERTV3_TRACK_THRESHOLD" "$CERTV3_SPATIAL_PENALTY" "$CERTV3_METRIC_DIM" "$CERTV3_FRAME_COVERAGE_RATIO" "$CERTV3_CELL_COVERAGE_RATIO" "$CERTV3_QUERY_THRESHOLD" "$CERTV3_QUERY_PER_ATOM" "$CERTV3_STRUCTURAL_WEIGHT" "$CERTV3_WHITENING_STRENGTH" "$CERTV3_QUALITY_FLOOR" "$CERTV3_RIDGE" "$CERTV3_SWAP_STEPS" "$CERTV3_SWAP_POOL" "$CERTV3_SWAP_MARGIN" "$CERTV3_FUSION_ALPHA" "$CERTV3_ASSIGNMENT_TEMPERATURE" "$CERTSB_SEMANTIC_RATIO" "$CERTSB_TEMPORAL_RATIO" "$CERTSB_COVERAGE_RATIO" "$CERTSB_TEMPORAL_BINS" "$CERTSB_LOCAL_RADIUS" "$CERTSB_CHANGE_LOW_QUANTILE" "$CERTSB_CHANGE_PEAK_QUANTILE" "$CERTSB_CHANGE_HIGH_QUANTILE" "$CERTSB_SCENE_QUANTILE" "$CERTSB_SCENE_SUPPRESSION" "$CERTSB_TEMPORAL_QUERY_WEIGHT" "$CERTSB_PROTECT_STRUCTURED" "$CERTSB_DEBUG"
      ;;
    certvid_v4)
