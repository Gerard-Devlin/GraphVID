...
test -f "$DATASET_JSONL" || { echo "VideoMME JSONL not found: $DATASET_JSONL" >&2; exit 1; }
test -d "$VIDEO_ROOT" || { echo "Video root not found: $VIDEO_ROOT" >&2; exit 1; }
...
SEED="${SEED:-20260813}"
SCORE_FILE="${SCORE_FILE:-$ROOT/scripts/efficiency/llava_ov_cross_budget_scores.json}"
OUT="${OUT:-$ROOT/logs/efficiency/llavaov_cross_budget_$(date +%Y%m%d_%H%M%S)}"
RAW_DIR="$OUT/raw"
...
echo "============================================================"
echo "LLaVA-OneVision cross-budget efficiency benchmark"
echo "Vanilla=100% FlashVID=1% Ours=0.5%"
...
METHOD_RATES=("vanilla:1.0" "flashvid:0.01" "ours:0.005")
for ENTRY in "${METHOD_RATES[@]}"; do
  METHOD="${ENTRY%%:*}"
  RATE="${ENTRY##*:}"
  echo "============================================================"
  echo "[$(date)] starting method=$METHOD retention=$RATE"
  echo "============================================================"
  CUDA_VISIBLE_DEVICES="$GPU" \
    "$PYTHON_BIN" playground/bench_efficiency.py run \
      --method "$METHOD" \
      --retention-ratio "$RATE" \
...
"$PYTHON_BIN" playground/bench_efficiency.py summarize \
  ...
  --methods vanilla flashvid ours \
  --num-warmup "$NUM_WARMUP" \
  --num-repeats "$NUM_REPEATS"

echo "============================================================"
echo "Cross-budget benchmark complete: $OUT"
