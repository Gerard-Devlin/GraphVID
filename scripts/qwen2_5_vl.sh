...
# released Qwen2.5 evaluation script. Set GEN_KWARGS explicitly to override.
...
# Qwen-aware full-feature kernel CertVID defaults.
QCERT_EXPANSION="${QCERT_EXPANSION:-1.25}"
QCERT_PRUNING_LAYER="${QCERT_PRUNING_LAYER:-20}"
QCERT_LLM_RETENTION_RATIO="${QCERT_LLM_RETENTION_RATIO:-0.3}"
QCERT_BUDGET_USES_EXPANSION="${QCERT_BUDGET_USES_EXPANSION:-True}"
QCERT_QUERY_ATOMS="${QCERT_QUERY_ATOMS:-8}"
QCERT_TEMPORAL_BINS="${QCERT_TEMPORAL_BINS:-0}"
QCERT_SPATIAL_BINS="${QCERT_SPATIAL_BINS:-0}"
QCERT_CANDIDATE_MULTIPLIER="${QCERT_CANDIDATE_MULTIPLIER:-2.5}"
QCERT_TRACK_THRESHOLD="${QCERT_TRACK_THRESHOLD:-0.82}"
QCERT_TRACK_SPATIAL_PENALTY="${QCERT_TRACK_SPATIAL_PENALTY:-0.08}"
QCERT_FRAME_COVERAGE_RATIO="${QCERT_FRAME_COVERAGE_RATIO:-1.0}"
QCERT_CELL_COVERAGE_RATIO="${QCERT_CELL_COVERAGE_RATIO:-0.35}"
QCERT_QUERY_THRESHOLD="${QCERT_QUERY_THRESHOLD:-0.10}"
QCERT_QUERY_PER_ATOM="${QCERT_QUERY_PER_ATOM:-1}"
QCERT_QUALITY_QUERY_WEIGHT="${QCERT_QUALITY_QUERY_WEIGHT:-0.12}"
QCERT_WHITENING_STRENGTH="${QCERT_WHITENING_STRENGTH:-0.25}"
QCERT_SEMANTIC_WEIGHT="${QCERT_SEMANTIC_WEIGHT:-0.68}"
QCERT_PHASE_WEIGHT="${QCERT_PHASE_WEIGHT:-0.14}"
QCERT_TEMPORAL_WEIGHT="${QCERT_TEMPORAL_WEIGHT:-0.06}"
QCERT_SPATIAL_WEIGHT="${QCERT_SPATIAL_WEIGHT:-0.04}"
QCERT_SIGNAL_WEIGHT="${QCERT_SIGNAL_WEIGHT:-0.04}"
QCERT_DESIGN_QUERY_WEIGHT="${QCERT_DESIGN_QUERY_WEIGHT:-0.04}"
QCERT_PHASE_LEVELS="${QCERT_PHASE_LEVELS:-4}"
QCERT_QUALITY_FLOOR="${QCERT_QUALITY_FLOOR:-0.15}"
QCERT_RIDGE="${QCERT_RIDGE:-0.50}"
QCERT_KERNEL_TOLERANCE="${QCERT_KERNEL_TOLERANCE:-0.0001}"
QCERT_MAX_KERNEL_PIVOTS="${QCERT_MAX_KERNEL_PIVOTS:-1024}"
QCERT_FUSION_ALPHA="${QCERT_FUSION_ALPHA:-0.08}"
QCERT_FUSION_SIMILARITY="${QCERT_FUSION_SIMILARITY:-0.82}"
QCERT_FUSION_TEMPORAL_RADIUS="${QCERT_FUSION_TEMPORAL_RADIUS:-1.0}"
QCERT_FUSION_SPATIAL_RADIUS="${QCERT_FUSION_SPATIAL_RADIUS:-2.0}"
QCERT_ASSIGNMENT_TEMPERATURE="${QCERT_ASSIGNMENT_TEMPERATURE:-0.07}"
...
common_flash_args() {
  ...
  case "$method" in
    ...
    certvid_v3)
      ...
      ;;
    certvid_qwen)
      expansion="$QCERT_EXPANSION"
      pruning_layer="$QCERT_PRUNING_LAYER"
      llm_retention_ratio="$QCERT_LLM_RETENTION_RATIO"
      ;;
    fastvid|visionzip)
      ;;
    *)
...
method_flash_args() {
  ...
  case "$method" in
    ...
    certvid_v3)
      ...
      ;;
    certvid_qwen)
      printf 'compression_variant=certvid_qwen,qcert_budget_uses_expansion=%s,qcert_query_atoms=%s,qcert_temporal_bins=%s,qcert_spatial_bins=%s,qcert_candidate_multiplier=%s,qcert_track_threshold=%s,qcert_track_spatial_penalty=%s,qcert_frame_coverage_ratio=%s,qcert_cell_coverage_ratio=%s,qcert_query_threshold=%s,qcert_query_per_atom=%s,qcert_quality_query_weight=%s,qcert_whitening_strength=%s,qcert_semantic_weight=%s,qcert_phase_weight=%s,qcert_temporal_weight=%s,qcert_spatial_weight=%s,qcert_signal_weight=%s,qcert_design_query_weight=%s,qcert_phase_levels=%s,qcert_quality_floor=%s,qcert_ridge=%s,qcert_kernel_tolerance=%s,qcert_max_kernel_pivots=%s,qcert_fusion_alpha=%s,qcert_fusion_similarity=%s,qcert_fusion_temporal_radius=%s,qcert_fusion_spatial_radius=%s,qcert_assignment_temperature=%s' \
        "$QCERT_BUDGET_USES_EXPANSION" "$QCERT_QUERY_ATOMS" \
        "$QCERT_TEMPORAL_BINS" "$QCERT_SPATIAL_BINS" \
        "$QCERT_CANDIDATE_MULTIPLIER" "$QCERT_TRACK_THRESHOLD" \
        "$QCERT_TRACK_SPATIAL_PENALTY" "$QCERT_FRAME_COVERAGE_RATIO" \
        "$QCERT_CELL_COVERAGE_RATIO" "$QCERT_QUERY_THRESHOLD" \
        "$QCERT_QUERY_PER_ATOM" "$QCERT_QUALITY_QUERY_WEIGHT" \
        "$QCERT_WHITENING_STRENGTH" "$QCERT_SEMANTIC_WEIGHT" \
        "$QCERT_PHASE_WEIGHT" "$QCERT_TEMPORAL_WEIGHT" \
        "$QCERT_SPATIAL_WEIGHT" "$QCERT_SIGNAL_WEIGHT" \
        "$QCERT_DESIGN_QUERY_WEIGHT" "$QCERT_PHASE_LEVELS" \
        "$QCERT_QUALITY_FLOOR" "$QCERT_RIDGE" "$QCERT_KERNEL_TOLERANCE" \
        "$QCERT_MAX_KERNEL_PIVOTS" \
        "$QCERT_FUSION_ALPHA" "$QCERT_FUSION_SIMILARITY" \
        "$QCERT_FUSION_TEMPORAL_RADIUS" "$QCERT_FUSION_SPATIAL_RADIUS" \
        "$QCERT_ASSIGNMENT_TEMPERATURE"
      ;;
    *)
      echo "Unsupported Qwen2.5 method: $method" >&2
      return 1
